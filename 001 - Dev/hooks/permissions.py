#!/usr/bin/python
# Prod & Dev site hook
# Run by the post-checkout hook to restore file permissions after transfer/after reversion
...
permissions = []
file = '/home/devuser/public_html/transfer/.permissions'
...
for perm in permissions:
    ...
    if os.path.exists(perm[PER.Path.value]):
        ...
        if perm[PER.Owner.value] == "root":
            ...
            os.chown(perm[PER.Path.value], uid_devuser, gid_nobody)

        else:

            os.chown(perm[PER.Path.value], uid_devuser, gid_devuser)
